 ...
 * Module: activity
 *
 * Was the standalone plugin `os-activity`. Merged into the OS plugin; the
...
final class Core_Index_Activity {
	...
	const REST_NS   = 'activity/v1';
	const CATEGORY  = 'os-activity';
	const TYPES     = array (
  'activity' => 
  array (
	...
	public static function register_ability_category(): void {
		wp_register_ability_category(
			...
			array(
				'label'       => 'OS Activity',
				'description' => 'Abilities owned by OS Activity.',
			)
		);
	}
	...
	private static function ability( string $id, string $label, array $input, callable $permission, callable $execute ): void {
		wp_register_ability(
			...
			array(
				'label'               => $label,
				'description'         => $label . ' in OS Activity.',
