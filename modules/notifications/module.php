 ...
 *
 * Was the standalone plugin `os-notifications`. Merged into the OS plugin; the
...
final class OS_Notifications {
	...
	const REST_NS   = 'notifications/v1';
	const CATEGORY  = 'os-notifications';
	const TYPES     = array (
  'notification' => 
  array (
	...
	public static function register_ability_category(): void {
		wp_register_ability_category(
			...
			array(
				...
				'description' => 'Abilities owned by OS Notifications.',
			)
		);
	}
	...
	private static function ability( string $id, string $label, array $input, callable $permission, callable $execute ): void {
		wp_register_ability(
			...
			array(
				'label'               => $label,
				'description'         => $label . ' in OS Notifications.',
